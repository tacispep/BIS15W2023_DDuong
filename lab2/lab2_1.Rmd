...
```{r}
A_avg_top_speed <- ((venom_GT + chiron + veyron)/3)
A_avg_top_speed
```

3. Use the function `mean()` to calculate the mean top speed for the cars.
```{r}
top_speed_list <- c(venom_GT, chiron, veyron)
M_avg_top_speed <- (mean(top_speed_list))
M_avg_top_speed
```
...
```
useful because it removed any null values, allowing the mean to properly calculate.
